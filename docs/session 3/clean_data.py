...
def clip_negative_pm25(df: pd.DataFrame) -> pd.DataFrame:
    ...
    assert (out["pm25"] >= 0).all() | out["pm25"].isna().any(), \
...
def assert_clean_invariants(df: pd.DataFrame) -> None:
    ...
    """
    assert df["pm25_mean"] is not None, "pm25_mean column missing"
...
def clean_aqicn_dataset(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    np.random.seed(42)  # reproducibility: seed even if unused now
    df = (
        df_raw
    ...
    df = df.sort_values(["station_id", "year_month"]).reset_index(drop=True)
